...
public class ClassMaster implements IVoicedCommandHandler,IBBSHandler {
	...
	public String process(L2PcInstance player, String args, boolean isBBS) { 
		if (player.getGameEvent()!=null || Olympiad.getInstance().isRegistered(player) ||
				...
				if (((level >= 20 && jobLevel == 0) || (level >= 40 && jobLevel == 1) || (level >= 76 && jobLevel == 2)) && Config.CLASS_MASTER_SETTINGS.isAllowed(newJobLevel))
				{
					...
					for (ClassId child : ClassId.values())
					{
						if (child.childOf(classId) && child.level() == newJobLevel)
							sb.append("<br><a action=\"bypass ").append(isBBS ? "_bbschange_class " : "voice_classmaster ").append(child.getId()).append("\"> ").append(CharTemplateTable.getClassNameById(child.getId())).append("</a>");
					}
		...
		sb.append("Поздравляю, Вы получили класс: <font color=\"LEVEL\">").append(CharTemplateTable.getClassNameById(player.getClassId().getId())).append("</font>.");
		sb.append("</center></body></html>");
